...
class DatabaseManager:
    ...
    def execute_query(self, query: str, params: Tuple = (), commit: bool = False) -> Optional[List[Tuple]]:
...
class ProductRepository:
    ...
    def get_all(self) -> List[Product]:
        ...
        results = self.db.execute_query("SELECT nombre, cantidad, precio, costo FROM productos")
    ...
    def get_by_name(self, nombre: str) -> Optional[Product]:
        ...
        result = self.db.execute_query("SELECT nombre, cantidad, precio, costo FROM productos WHERE nombre = ?", (nombre,))
    ...
    def remove(self, nombre: str) -> None:
        """Elimina un producto"""
        self.db.execute_query("DELETE FROM productos WHERE nombre = ?", (nombre,), commit=True)
...
class SalesRepository:
    ...
    def get_current_sales(self) -> List[SaleItem]:
        ...
        results = self.db.execute_query("SELECT producto, cantidad, precio_unitario, subtotal FROM ventas_actuales")
    ...
    def add_to_current_sales(self, nombre: str, cantidad: int = 1) -> None:
        ...
        existing = next((item for item in self.get_current_sales() if item.producto == nombre), None)
    ...
    def process_sale(self) -> float:
        ...
        self.db.execute_query(
            "UPDATE totales SET total_ventas = total_ventas + ?",
            (total,),
            commit=True
        )
...
class TotalsRepository:
    ...
    def get_totals(self) -> Totals:
        ...
        result = self.db.execute_query("SELECT total_ventas, total_gastado FROM totales")
...
class SaleDetailsPanel(tb.Frame):
    ...
    def _setup_ui(self) -> None:
        ...
        tb.Button(
            ...
            command=self.master.show_totals, width=15
        ).pack(pady=10)
...
class ProductForm(tb.Frame):
    ...
    def _setup_form(self) -> None:
        ...
        tb.Label(self, text="Agregar/Editar Producto", font=('Helvetica', 12, 'bold')).pack(pady=5)
        ...
        tb.Button(
            ...
            command=self.master.show_sales_view
        ).pack(side=tk.LEFT, padx=5)
...
class SalesPanel(tb.Frame):
    ...
    def _setup_ui(self) -> None:
        """Configura los componentes de la interfaz"""
        tb.Label(self, text="Productos en Venta", font=('Helvetica', 12, 'bold')).pack(pady=5)
        ...
        tb.Label(total_frame, text="Total Venta:", font=('Helvetica', 10, 'bold')).pack(side=tk.LEFT, padx=5)
        self.total_label = tb.Label(total_frame, text="$0.00", font=('Helvetica', 10, 'bold'))
...
class ProductTable(tb.Frame):
    ...
    def _on_product_selected(self, event) -> None:
        ...
        try:
            ...
            added_item = next(item for item in current_sales if item.producto == product_name)
...
class InventoryApp(tb.Window):
    ...
    def _setup_ui(self) -> None:
        ...
        self.product_form = ProductForm(self.right_panel)
